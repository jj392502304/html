...
$(function(){
	...
	$(".product-del").click(function(){
		if(confirm("您确定要删除当前商品？")){
            var current=$(this).closest(".product-box")
			var t=$(this).parent().children()[2];
			var name=$(t).find("input")[0].name
            $.ajax({
                type: "POST",
                url: http + "/cart/delete",
                data: {
                    "id":name
                },
//            contentType: "application/json",
	...
	$("#del_all").click(function () {
        if(confirm("您确定要删除已选商品？")){
			var xz=$(".product-xz").closest(".product-box");
            $.ajax({
                type: "POST",
                url: http + "/cart/deleteids",
                data: {
                    "ids":ids
                },
//            contentType: "application/json",
...
function shuliang(){
	ids="";
   var t= $(".product-xz").closest(".product-box");
	$.each(t,function (n,value) {
		ids+="\'"+$(value).find("input")[0].name+"\'"+",";
    })
	...
	if(cd>0){
		...
		$("#del_all").css("display","inline-block");
	}else{
...
function next() {
    // window.parent.location.href = "'.site_url($this->m . '/login/index').'";
    window.location.href = "../orderform/index.html?id="+userid+"&ids="+ids;
